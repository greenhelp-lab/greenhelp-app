...
if ($servicos) {
  foreach ($servicos as $servico) {
    ...
    $areaImg = $servico['area_img'] ?? ''; // link da imagem da área

    echo '
        <div class="service-card" data-area="' . htmlspecialchars($areaNome) . '" onclick="window.location.href=\'sobre_servico.php?id=' . $servico['id'] . '\'">
            <div class="service-header">
                <h4>' . htmlspecialchars($servico['nome']) . '</h4>
                <img class="service-card-icon" src="' . htmlspecialchars($areaImg) . '" alt="' . htmlspecialchars($areaNome) . '" style="width:40px; height:40px;">
            </div>
